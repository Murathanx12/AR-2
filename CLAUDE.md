...
- Piezo buzzer (GPIO46)
- 3x HC-SR04 ultrasonic: Left (GPIO19 trig, GPIO20 echo), Center (GPIO1 trig, GPIO18 echo), Right (GPIO4 trig, GPIO48 echo) ⚠ GPIO48 shared with NeoPixel
...
- `REROUTING` is a **hybrid manoeuvre**. On first tick, `_obstacle_is_close()` picks between a smooth curve (for far obstacles) and a discrete rotate-drive-rotate (for close ones). The close/far cue combines three signals, any one of which flags "close": ultrasonic < 50 cm (if wired), obstacle bbox bottom > 82 % of frame height (ground-plane heuristic), or obstacle bbox area > 18 % of frame area.
- **Far-obstacle path (`curve` phase):** `vx=22, vy=omega_sign*8, omega=omega_sign*12` — smooth mecanum arc with forward motion + rotation + small strafe. Camera stays mostly forward so the tag often remains in peripheral view. If `_obstacle_is_close()` flips True mid-curve (we got too close to safely arc) → stops, switches to `turn_away`. Per-phase timeout 3.5 s also switches to rotate-drive-rotate.
- **Close-obstacle path (rotate-drive-rotate sub-FSM):**
  1. `turn_away` — rotate in place to push the obstacle to the far frame edge. Obstacle on right half → `omega=-25` (CCW; obstacle drifts right off-screen). Left half → `omega=+25`. Done when obstacle cx is past the 15 % / 85 % edge, is no longer detected, or 1.8 s elapse.
  2. `drive_around` — `vx=25, vy=0, omega=0` moving forward with the camera leading. Exits after the path stays clear for ~5 consecutive frames *and* a minimum drive time of 0.8 s (prevents peek-past false exits), or on a 3 s per-phase timeout. A new obstacle mid-drive falls back to `turn_away` with a freshly-chosen rotation direction.
  3. `turn_back` — rotate the **opposite** direction looking for the target marker. Marker reappears → straight back to `ARUCO_APPROACH`. 2.5 s timeout → hand off to `ARUCO_SEARCH` which further biases the spin by `_aruco_last_cx`.
- **Side-choice logic** (`_choose_reroute_side`) runs on entry to REROUTING and again if a new obstacle appears during `drive_around`. It weighs three signals in priority order:
  1. **Lateral clearance** — distance from the main obstacle's left / right edge to the frame edge, shrunk by any secondary obstacle sitting in that escape lane. If one side has ≥1.5× the clearance of the other, that side wins outright.
  2. **Tag bearing memory** — if `_aruco_last_cx` is fresh (<10 s) and off-centre, the robot prefers the side the tag is on (shorter path back). Used as a tiebreaker when clearances are close.
  3. **Geometric fallback** — obstacle on right half → pass left; obstacle on left half → pass right.
- Returns `-1` (pass on the left, rotate CCW) or `+1` (pass on the right, rotate CW). Same sign convention as line-follower.
- Overall 8 s timeout across all phases; says "I cannot find a way around" and goes to `IDLE` if it fires. Ultrasonic always preempts to `BLOCKED`.
